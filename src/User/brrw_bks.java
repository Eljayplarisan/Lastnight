...
public class brrw_bks extends javax.swing.JFrame {
    ...
    private void loadBooks() {
        ...
        try (Connection conn = config.connectDB();
            ...
            if (!books.isEmpty()) {
                ...
                selectedStatus = books.get(0).status;
    ...
    private void initComponents() {
        ...
        borrow.setFont(new java.awt.Font("Tahoma", 1, 18));
    ...
    private void borrowSelectedBook() {
        ...
        if (username == null || username.trim().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Unable to detect the logged-in user.");
            return;
        }
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this, "Unable to borrow the selected book: " + e.getMessage());
        } finally {
            if (conn != null) {
                try {
    ...
    private String fetchUserEmail(String username) {
        ...
        try (Connection conn = config.connectDB();
             PreparedStatement pst = conn.prepareStatement("SELECT u_email FROM tbl_data WHERE u_uname = ? LIMIT 1")) {
    ...
    private String getSelectedBookImage() {
        for (BookItem item : books) {
            if (item.id == selectedBookId) {
